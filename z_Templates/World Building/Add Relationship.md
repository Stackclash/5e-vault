...
if (tp.config.run_mode !== 1) {
    ...
    const selectedNpc = await tp.system.suggester(parentLocations.map(n => n.file.name), parentLocations.map(p => [p.file.path, p.file.name]), false, "What NPC should be used for the relationship?")
