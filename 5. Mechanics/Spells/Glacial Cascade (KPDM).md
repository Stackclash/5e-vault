---
obsidianUIMode: preview
cssclasses: json5e-spell
tags:
  - spell
  - compendium/src/5e/kpdm
  - spell/class/bard/magical-secrets
  - spell/class/wizard
  - spell/level/8th-level
  - spell/school/evocation
classes:
  - Bard (Magical Secrets)
  - Wizard
school: Evocation
level: 8
ritual: false
components: 'V, S, M (a piece of alexandrite)'
verbal: true
somatic: true
material: true
material_desc: a piece of alexandrite
time: 1 Action
duration: Instantaneous
range: Self (30-foot Sphere)
aliases:
  - Glacial Cascade
damage:
  base: 10d8
  scaling:
    damage: null
    level: null
  types:
    - cold
saving_throw:
  skills:
    - Constitution
  succeeds: see spell description
area:
  shape: sphere
  size: 30
---
# Glacial Cascade
*8th-level, Evocation*  

- **Casting time:** 1 Action
- **Range:** Self (30-foot Sphere)
- **Components:** V, S, M (a piece of alexandrite)
- **Duration:** Instantaneous

By harnessing the power of ice and frost, you emanate pure cold, filling a 30-foot-radius sphere. Creatures other than you in the sphere take `dice:10d8|noform|noparens|avg` (`10d8`) cold damage, or half as much damage with a successful Constitution saving throw. A creature killed by this spell is transformed into ice, leaving behind no trace of its original body.

**Classes**: [[/5. Mechanics/Lists/List Spells Classes Bard.md\|Bard (Magical Secrets)]]; [[/5. Mechanics/Lists/List Spells Classes Wizard.md\|Wizard]]

*Source: Deep Magic p. 272*
