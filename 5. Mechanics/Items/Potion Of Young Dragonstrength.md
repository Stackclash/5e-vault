---
obsidianUIMode: preview
cssclasses: json5e-item
tags:
  - compendium/src/5e/hhhvi
  - item/property/crfby
  - item/rarity/rare
  - item/wondrous/potion
aliases:
  - Potion of Young Dragonstrength
---
# Potion of Young Dragonstrength
*Potion, rare, Crafted by*  

- **Properties**: Crafted by

When you drink this potion, you recover `dice: 6d4 + 6|avg` (`6d4 + 6`) hit points. In addition, you gain resistance to the damage type of the dragon it was crafted from for 1 hour.

> [!embed-table]- Dragon Types
> ![[5. Mechanics/Tables/Dragon Types.md\|Dragon Types]]

*Source: Hamund's Harvesting Handbook: Volume I p. 137*
