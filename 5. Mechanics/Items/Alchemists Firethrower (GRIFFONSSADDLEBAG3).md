...
The device loses 1 charge each time you use it to make a ranged weapon attack, which has a normal range of 30 feet and a long range of 90 feet. On a hit, a target takes fire damage equal to `dice:1d6|noform|noparens|avg` (`1d6`) plus your Dexterity modifier. For each attack that hits, the target also takes `dice:1d6|noform|noparens|avg` (`1d6`) fire damage at the end of its next turn as the alchemist's fire smolders and evaporates from its body.
