...
This once-rusted cavalry saber was reforged with mourningsteel scavenged from the same battlefield where it was separated from its long-dead owner. Attacks with this weapon deal slashing damage instead of piercing damage, and when you hit with it, the target takes an extra `dice:1d6|noform|noparens|avg` (`1d6`) necrotic damage. If you're mounted, the extra necrotic damage increases to `dice:1d8|noform|noparens|avg` (`1d8`).
