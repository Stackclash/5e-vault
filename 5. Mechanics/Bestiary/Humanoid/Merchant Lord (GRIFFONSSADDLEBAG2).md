...
"actions":
  ...
  - "desc": "The merchant enchants a coin in its hand with divination magic, which\
      \ lasts for 10 minutes or until the merchant ends the effect early (no action\
      \ required). For the duration, a creature holding the coin that knows at least\
      \ one language can understand the meaning of any word the merchant speaks, and\
      \ when the merchant speaks, that creature understands what the merchant says.\n\
      \nAlternatively, the merchant can use an action to see and hear through the\
      \ enchanted coin until the start of its next turn, using its own senses as if\
      \ it were in the coin's space. During this time, the merchant is [[/5. Mechanics/Rules/Conditions.md#Deafened|deaf]]\
