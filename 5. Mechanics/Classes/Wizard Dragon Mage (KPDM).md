...
Starting at 2nd level, you can invoke a magic Dragon mask. You use a bonus action and expend a spell slot to invoke the mask, which lasts for 3 rounds for each level of the expended spell slot. The dragon mask remains as long as you are not [[/5. Mechanics/Rules/Conditions.md#Incapacitated\|incapacitated]], until you use a bonus action to dismiss or replace it, or until the duration expires. The mask is a translucent magical force, in the form of a dragon's head, which covers your face. Your face is still visible through the mask, and the mask does not hinder vision.
...
- You gain a bite attack. As an action, make a melee spell attack against one adjacent target. On a hit, the target takes piercing damage equal to `dice:1d8|noform|noparens|avg` (`1d8`) + your Intelligence modifier. This attack counts as magical for the purpose of overcoming a target's immunity or resistance to piecing damage.  
...
- As a bonus action, you can expend an additional spell slot to augment the damage of your next Dragon mask bite attack. The damage increases by `dice:1d8|noform|noparens|avg` (`1d8`) for each level of the expended spell slot, and you have advantage on the attack roll.  
