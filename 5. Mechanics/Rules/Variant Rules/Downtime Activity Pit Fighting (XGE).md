...
The character makes three checks: Strength ([[/5. Mechanics/Rules/Skills.md#Athletics|")"]], Dexterity ([[/5. Mechanics/Rules/Skills.md#Acrobatics|")), and a special Constitution check that has a bonus equal to a roll of the character's largest Hit Die (this roll doesn't spend that die"]]. If desired, the character can replace one of these skill checks with an attack roll using one of the character's weapons. The DC for each of the checks is `dice: 5 + 2d10|avg` (`5 + 2d10`); generate a separate DC for each one. Consult the Pit Fighting Results table to see how the character did.
