...
- Quests borrow lore; they don't store it. When a quest, arc, or planning discussion introduces a new NPC or location — or adds real detail to an existing one — offer to file that into its own note via the `world-builder` and `vault-note` skills, and reference it from the quest or prep text with a wikilink. Don't leave the only copy of an NPC's or location's details inside quest or session-prep text.
