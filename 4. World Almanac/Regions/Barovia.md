...
>  WHERE file.frontmatter.region_PoI = this.file OR file.frontmatter.region_settlement = this.file
> ```
...
```dataview
TABLE 
FROM "4. World Almanac/Settlements"
WHERE contains(file.region_settlement, this.file.name)
```