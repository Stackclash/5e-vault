...
| 4 | The spell's effect is contrary to its normal one, but neither harmful nor beneficial. For instance, a [fireball](/6. Mechanics/spells/fireball.md) might produce an area of harmless cold. |
| 5 | The caster suffers a minor but bizarre effect related to the spell. Such effects last only as long as the original spell's duration, or `dice: 1d10|avg` (`1d10`) minutes for spells that take effect instantaneously. For example, a [fireball](/6. Mechanics/spells/fireball.md) might cause smoke to billow from the caster's ears for `dice: 1d10|avg` (`1d10`) minutes.  |
