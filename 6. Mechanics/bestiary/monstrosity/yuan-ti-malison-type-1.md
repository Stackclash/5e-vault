...
![[token/yuan-ti-malison-type-1.png#token]]
...
***Bite.*** *Melee Weapon Attack:* `dice: d20+5` (+5 to hit), reach 5 ft., one creature. Hit: `dice: 1d4 + 3|avg` (`1d4 + 3`) piercing damage plus `dice: 2d6|avg` (`2d6`) poison damage.
...
***Longbow (Yuan-ti Form Only).*** *Ranged Weapon Attack:* `dice: d20+4` (+4 to hit), range 150/600 ft., one target. Hit: `dice: 1d8 + 2|avg` (`1d8 + 2`) piercing damage plus `dice: 2d6|avg` (`2d6`) poison damage.
```
^statblock
