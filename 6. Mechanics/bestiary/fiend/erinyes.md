...
> 
> Some erinyes carry a [rope of entanglement](/6. Mechanics/items/rope-of-entanglement.md) (detailed in the Dungeon Master's Guide). When such an erinyes uses its Multiattack, the erinyes can use the rope in place of two of the attacks.
...
***Hellish Weapons.*** The erinyes's weapon attacks are magical and deal an extra 13 (`dice: 3d8|avg` (`3d8`)) poison damage on a hit (included in the attacks).
...
***Longsword.*** *Melee Weapon Attack:* `dice: d20+8` (+8 to hit), reach 5 ft., one target. Hit: `dice: 1d8 + 4|avg` (`1d8 + 4`) slashing damage, or 9 (`dice: 1d10 + 4|avg` (`1d10 + 4`)) slashing damage if used with two hands, plus `dice: 3d8|avg` (`3d8`) poison damage.

***Longbow.*** *Ranged Weapon Attack:* `dice: d20+7` (+7 to hit), range 150/600 ft., one target. Hit: `dice: 1d8 + 3|avg` (`1d8 + 3`) piercing damage plus `dice: 3d8|avg` (`3d8`) poison damage, and the target must succeed on a DC 14 Constitution saving throw or be [poisoned](/6. Mechanics/rules/conditions.md#poisoned). The poison lasts until it is removed by the [lesser restoration](/6. Mechanics/spells/lesser-restoration.md) spell or similar magic.
