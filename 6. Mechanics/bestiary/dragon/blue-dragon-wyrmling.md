...
![[token/blue-dragon-wyrmling.png#token]]
...
***Bite.*** *Melee Weapon Attack:* `dice: d20+5` (+5 to hit), reach 5 ft., one target. Hit: `dice: 1d10 + 3|avg` (`1d10 + 3`) piercing damage plus `dice: 1d6|avg` (`1d6`) lightning damage.

***Lightning Breath (Recharge 5-6).*** The dragon exhales lightning in a 30-foot line that is 5 feet wide. Each creature in that line must make a DC 12 Dexterity saving throw, taking 22 (`dice: 4d10|avg` (`4d10`)) lightning damage on a failed save, or half as much damage on a successful one.
```
^statblock