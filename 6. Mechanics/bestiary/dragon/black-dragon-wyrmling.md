...
![[token/black-dragon-wyrmling.png#token]]
...
***Bite.*** *Melee Weapon Attack:* `dice: d20+4` (+4 to hit), reach 5 ft., one target. Hit: `dice: 1d10 + 2|avg` (`1d10 + 2`) piercing damage plus `dice: 1d4|avg` (`1d4`) acid damage.

***Acid Breath (Recharge 5-6).*** The dragon exhales acid in a 15-foot line that is 5 feet wide. Each creature in that line must make a DC 11 Dexterity saving throw, taking 22 (`dice: 5d8|avg` (`5d8`)) acid damage on a failed save, or half as much damage on a successful one.
```
^statblock