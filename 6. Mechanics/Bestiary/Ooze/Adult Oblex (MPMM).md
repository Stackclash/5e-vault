...
While memory drained, the target must roll a `dice: d4\|avg` (`d4`) and subtract the number rolled from its ability checks and attack rolls. Each time the target is memory drained beyond the first, the die size increases by one: the `dice: d4\|avg` (`d4`) becomes a `dice: d6\|avg` (`d6`), the `dice: d6\|avg` (`d6`) becomes a `dice: d8\|avg` (`d8`), and so on until the die becomes a `dice: d20\|avg` (`d20`), at which point the target becomes [[6. Mechanics/Rules/conditions.md#unconscious\|unconscious]] for 1 hour. The effect then ends.
