...
On each of its turns, the giant uses all its movement to move toward the nearest creature or whatever else it might perceive as food. Roll a `dice: d10\|avg` (`d10`) at the start of each of the giant's turns to determine its action for that turn:
...
***Bite.*** *Melee Weapon Attack:* `dice: d20+8` (+8 to hit), reach 5 ft., one creature. Hit: `dice: 3d6 + 5\|avg` (`3d6 + 5`) piercing damage, and the giant magically regains hit points equal to the damage dealt.

***Fist.*** *Melee Weapon Attack:* `dice: d20+8` (+8 to hit), reach 10 ft., one target. Hit: `dice: 3d8 + 5\|avg` (`3d8 + 5`) bludgeoning damage.
```
^statblock
