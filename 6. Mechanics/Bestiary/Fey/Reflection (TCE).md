...
```ad-statblock
title: Reflection
![[/6. Mechanics/Bestiary/Fey/Token/reflection.png#token]]
...
***Strength Drain.*** *Melee Weapon Attack:* `dice: d20+4` (+4 to hit), reach 5 ft., one creature. Hit: `dice: 2d6 + 2` (`2d6 + 2`) necrotic damage, and the target's Strength score is reduced by `dice: 1d4` (`1d4`). The target dies if this reduces its Strength to 0. Otherwise, the reduction lasts until the target finishes a short or long rest.

If a non-evil humanoid dies from this attack, a new reflection rises from the corpse `dice: 1d4` (`1d4`) hours later.
```
^statblock
