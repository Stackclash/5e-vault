...
***Quarterstaff.*** *Melee Weapon Attack:* `dice: d20+3` (+3 to hit), reach 5 ft., one target. Hit: `dice: 1d6 + 1\|avg` (`1d6 + 1`) bludgeoning damage, or 5 (`dice: 1d8 + 1\|avg` (`1d8 + 1`)) bludgeoning damage if used with two hands, plus `dice: 3d8\|avg` (`3d8`) force damage.

***Devastating Bolt.*** *Ranged Spell Attack:* `dice: d20+5` (+5 to hit), range 60 ft., one target. Hit: `dice: 4d8 + 3\|avg` (`4d8 + 3`) force damage, and the target is knocked [[6. Mechanics/Rules/conditions.md#prone\|prone]].
```
^statblock
