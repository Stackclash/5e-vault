...
***Lightning Storm (Costs 2 Actions).*** Hutijin releases lightning in a 30-foot radius, blocked only by total cover. All other creatures in that area must each make a DC 22 Dexterity saving throw, taking 18 (`dice: 4d8\|avg` (`4d8`)) lightning damage on a failed save, or half as much damage on a successful one.
```
^statblock