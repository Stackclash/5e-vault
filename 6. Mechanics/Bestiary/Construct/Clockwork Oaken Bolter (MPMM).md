...
| dice: d8 | Malfunction |
|----------|-------------|
| 1 | **Faulty Sensors.** Roll a `dice: d6\|avg` (`d6`) at the start of the clock work's turn. If you roll a 1, the clockwork is [[6. Mechanics/Rules/conditions.md#blinded\|blinded]] until the end of its turn. |
...
| 7 | **Overheats.** Roll a `dice: d6\|avg` (`d6`) at the start of the clockwork's turn. If you roll a 1, the clockwork is [[6. Mechanics/Rules/conditions.md#incapacitated\|incapacitated]] until the end of its turn. |
...
![[/6. Mechanics/Bestiary/Construct/Token/clockwork-oaken-bolter.png#token]]
...
***Harpoon.*** *Ranged Weapon Attack:* `dice: d20+7` (+7 to hit), range 50/200 ft., one target. Hit: `dice: 1d10 + 4\|avg` (`1d10 + 4`) piercing damage, and the target is [[6. Mechanics/Rules/conditions.md#grappled\|grappled]] (escape DC 12). While [[6. Mechanics/Rules/conditions.md#grappled\|grappled]] in this way, a creature's speed isn't reduced, but it can move only in directions that bring it closer to the clockwork. A creature takes `dice: 1d10\|avg` (`1d10`) slashing damage if it escapes from the grapple or if it tries and fails. The clockwork can grapple only one creature at a time with its harpoon.
...
***Reel In.*** The clockwork pulls the creature [[6. Mechanics/Rules/conditions.md#grappled\|grappled]] by its Harpoon up to 20 feet closer.
```
^statblock
