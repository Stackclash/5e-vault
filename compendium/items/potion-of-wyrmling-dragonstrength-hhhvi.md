---
obsidianUIMode: preview
cssclasses: json5e-item
tags:
- compendium/src/5e/hhhvi
- item/property/crfby
- item/rarity/uncommon
- item/wondrous/potion
aliases: 
- "Potion of Wyrmling Dragonstrength"
---
# Potion of Wyrmling Dragonstrength
*Potion, uncommon, Crafted by*  

- **Properties**: Crafted by

When you drink this potion, you recover `dice: 3d4 + 3|avg` (`3d4 + 3`) hit points. In addition, you gain resistance to the damage type of the dragon it was crafted from for 1 hour.

> [!embed-table]- Dragon Types
> ![Dragon Types](compendium/tables/dragon-types-hhhvi.md)

*Source: Hamund's Harvesting Handbook: Volume I p. 137*