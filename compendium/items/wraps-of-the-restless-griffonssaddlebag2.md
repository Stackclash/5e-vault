...
In addition, when you take the [Attack](/compendium/rules/actions.md#Attack) action while wearing the wraps, you can use a bonus action immediately before or after one of your attacks to cause a piece of the wrapping to lash out at a target within 10 feet of you. Make a melee spell attack for the wrapping, using an attack bonus of +7. On a hit, the target takes `dice:1d8|noform|noparens|avg` (`d8`) necrotic damage.

The wraps have 3 charges and regain `dice:1d3|noform|noparens|avg` (`d3`) expended charges daily at dawn. As an action, you can hold out your hand and expend 1 or more of the wraps' charges to release a blast of sand from it in a 15-foot cone. Each creature in the area must make a DC 15 Constitution saving throw. On a failed save, a creature is [blinded](/compendium/rules/conditions.md#Blinded) until the start of your next turn, and for each charge you spend, it takes `dice:1d6|noform|noparens|avg` (`d6`) bludgeoning damage plus `dice:2d8|noform|noparens|avg` (`2d8`) necrotic damage. On a successful save, a creature takes half as much damage and isn't [blinded](/compendium/rules/conditions.md#Blinded).
