---
obsidianUIMode: preview
cssclasses: json5e-item
tags:
- compendium/src/5e/tgs1
- item/attunement/required
- item/rarity/very-rare
- item/tier/major
- item/weapon/simple
aliases: 
- Festerwood Fungal Stave
one-hand-damage: 1d6 bludgeoning
two-hand-damage: 1d8 bludgeoning
weight: 4.0
---
# Festerwood Fungal Stave
*Staff, weapon ([quarterstaff](/compendium/items/quarterstaff.md)), major, very rare (requires attunement by a cleric, druid, or sorcerer)*  
![](https://raw.githubusercontent.com/TheGiddyLimit/homebrew/master/_img/TGS1/Festerwood-Fungal-Stave.webp#right)  

- **Damage**:
  - One-handed: 1d6 bludgeoning
  - Two-handed: 1d8 bludgeoning
- **Properties**: [Versatile](/compendium/rules/item-properties.md#Versatile)
- **Weight**: 4.0 lbs.

This fungal staff has 25 charges and regains `dice:1d10+15|noform|noparens|avg` (`1d10 + 15`) expended charges daily at dusk. While holding the staff, you can use an action to expend 1 or more of its charges to cause clusters of noxious mushrooms to magically appear in a number of unoccupied spaces equal to the number of charges you expend. The spaces must be on solid ground within 60 feet of you. Each space becomes difficult terrain for the duration of the effect. When a creature moves into or within a space or area covered by these mushrooms, that creature takes `dice:2d4|noform|noparens|avg` (`2d4`) poison damage for every 5 feet it travels. The mushrooms live for 1 minute or until you dismiss them as a bonus action.

In addition, while holding the staff, you can use a bonus action to expend 1 or more of its charges to cast one of the following spells from it, regardless of the spell's normal casting time, using your spell save DC: [cloudkill](/compendium/spells/cloudkill.md) (5 charges), [fog cloud](/compendium/spells/fog-cloud.md) (1 charge), [insect plague](/compendium/spells/insect-plague.md) (5 charges), or [stinking cloud](/compendium/spells/stinking-cloud.md) (3 charges). A spell cast in this way ends at the end of your next turn and must target a point adjacent to a space occupied by your mushrooms. When cast from the staff, these spells do not require concentration.

If you expend the last charge from the staff, roll a `dice:d20|noform|noparens|avg` (`d20`). On a 1, the staff begins to overgrow, destroying the staff and forcing you to drop it in an unoccupied space adjacent to you. A hostile [violet fungus](/compendium/bestiary/plant/violet-fungus.md) grows in its place.

*Source: The Griffon's Saddlebag, Book 1 p. 62*