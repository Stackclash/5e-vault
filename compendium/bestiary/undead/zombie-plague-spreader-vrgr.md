...
- !!dev.ebullient.convert.tools.dnd5e.qute.Trait
  "desc": "Any creature that starts its turn within 10 feet of the plague spreader\
    ...
    \ is [poisoned](compendium/rules/conditions.md#poisoned) and can't regain hit\
...
![](/compendium/bestiary/undead/token/zombie-plague-spreader.png#token)
...
***Viral Aura.*** Any creature that starts its turn within 10 feet of the plague spreader must make a DC 12 Constitution saving throw. On a failed save, the creature is [poisoned](compendium/rules/conditions.md#poisoned) and can't regain hit points until the end of its next turn. On a successful save, the creature is immune to this plague spreader's Viral Aura for 24 hours.

## Actions

***Multiattack.*** The plague spreader makes two Slam attacks.

***Slam.*** *Melee Weapon Attack:* `dice: d20+5` (+5 to hit), reach 5 ft., one target. Hit: `dice: 1d6 + 3|avg` (`1d6 + 3`) bludgeoning damage plus `dice: 2d8|avg` (`2d8`) necrotic damage.

***Virulent Miasma (1/Day).*** The plague spreader releases toxic gas in a 30-foot-radius sphere centered on itself. Each creature in that area must make a DC 12 Constitution saving throw, taking 14 (`dice: 4d6|avg` (`4d6`)) poison damage on a failed save, or half as much damage on a successful one. A Humanoid reduced to 0 hit points by this damage dies and rises as a zombie (see its stat block in the Monster Manual) 1 minute later. The zombie acts immediately after the plague spreader in the initiative count.
```
^statblock