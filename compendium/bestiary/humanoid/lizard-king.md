...
```ad-statblock
title: Lizard King
![](/compendium/bestiary/humanoid/token/lizard-king.png#token)
...
***Skewer.*** Once per turn, when the lizardfolk makes a melee attack with its trident and hits, the target takes an extra 10 (`dice: 3d6|avg` (`3d6`)) damage, and the lizardfolk gains temporary hit points equal to the extra damage dealt.
...
***Trident.*** *Melee or Ranged Weapon Attack:* `dice: d20+5` (+5 to hit), reach 5 ft. or range 20/60 ft., one target. Hit: `dice: 1d6 + 3|avg` (`1d6 + 3`) piercing damage, or 7 (`dice: 1d8 + 3|avg` (`1d8 + 3`)) piercing damage if used with two hands to make a melee attack.
```
^statblock
