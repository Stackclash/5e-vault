...
```ad-statblock
title: Dao
![](/compendium/bestiary/elemental/token/dao.png#token)
...
***Sure-Footed.*** The dao has advantage on Strength and Dexterity saving throws made against effects that would knock it [prone](compendium/rules/conditions.md#prone).
...
***Maul.*** *Melee Weapon Attack:* `dice: d20+10` (+10 to hit), reach 5 ft., one target. Hit: `dice: 4d6 + 6|avg` (`4d6 + 6`) bludgeoning damage. If the target is a Huge or smaller creature, it must succeed on a DC 18 Strength check or be knocked [prone](compendium/rules/conditions.md#prone).
```
^statblock
