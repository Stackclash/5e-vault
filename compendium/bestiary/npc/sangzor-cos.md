...
```ad-statblock
title: Sangzor
![](/compendium/bestiary/npc/token/sangzor.png#token)
...
***Charge.*** If Sangzor moves at least 20 feet straight toward a target and then hits it with a ram attack on the same turn, the target takes an extra 5 (`dice: 2d4|avg` (`2d4`)) bludgeoning damage. If the target is a creature, it must succeed on a DC 13 Strength saving throw or be knocked [prone](compendium/rules/conditions.md#prone).

***Sure-Footed.*** Sangzor has advantage on Strength and Dexterity saving throws made against effects that would knock it [prone](compendium/rules/conditions.md#prone).

## Actions

***Ram.*** *Melee Weapon Attack:* `dice: d20+5` (+5 to hit), reach 5 ft., one target. Hit: `dice: 2d4 + 3|avg` (`2d4 + 3`) bludgeoning damage.
```
^statblock
