...
- !!dev.ebullient.convert.tools.dnd5e.qute.Trait
  "desc": "Melee Weapon Attack: dice: d20+5 (+5 to hit), reach 5 ft., one target.\
    ...
    \ it must succeed on a DC 13 Constitution saving throw or suffer one random effect\
    \ (roll a dice: d6|avg (d6)):\n\n- 1–2 Confusion. On its next turn, the\
    \ target must use its action to make one weapon attack against a random creature\
    \ it can see within 30 feet of it, using whatever weapon it has in hand and moving\
    \ beforehand if necessary to get in range. If it's holding no weapon, it makes\
    ...
    \ is [paralyzed](compendium/rules/conditions.md#paralyzed) until the end of its\
    \ next turn.  \n- 5–6 Poison. The target takes dice: 3d8|avg (3d8) poison\
    \ damage.  "
  "name": "Bite"
"source":
- "MPMM"
...
***Bite.*** *Melee Weapon Attack:* `dice: d20+5` (+5 to hit), reach 5 ft., one target. Hit: `dice: 1d6 + 3|avg` (`1d6 + 3`) piercing damage. If the target is a creature, it must succeed on a DC 13 Constitution saving throw or suffer one random effect (roll a `dice: d6|avg` (`d6`)):
...
- **3–4 Paralysis.** The target is [paralyzed](compendium/rules/conditions.md#paralyzed) until the end of its next turn.  
- **5–6 Poison.** The target takes `dice: 3d8|avg` (`3d8`) poison damage.  
```
^statblock
