...
Before a player begins hacking and butchering their hunt, they may instead choose to take a moment first and **appraise** the creature to be harvested. To do this, they must spend 1 minute examining the creature to be harvested and then roll an Intelligence check, adding their proficiency bonus if they are proficient in the skill corresponding to that creature (see table below). For example, appraising a [Beholder](compendium/bestiary/aberration/beholder.md) (which is an aberrant), the check would be an Intelligence ([Arcana](/compendium/rules/skills.md#Arcana)) check, while appraising an [Ogre](compendium/bestiary/giant/ogre.md) (which is a giant) would require an Intelligence ([Medicine](/compendium/rules/skills.md#Medicine)) check.
...
Each individual item in a creature's harvesting table is listed with a DC next to it. Any roll that a player makes that equals or exceeds this DC grants that player that item. Rewards are cumulative, and a player receives every item with a DC equal to or below their ability check result. For example, rolling a total of 15 on a check to harvest an [azer](compendium/bestiary/elemental/azer.md) will reward the player with both "azer ash", and "azer bronze skin", but not a "spark of creation". If they so wish, players may opt to not harvest a material even if they have met the DC threshold to harvest it.
