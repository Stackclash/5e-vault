...
- Enemies of the ranger and the locals must make DC 15 [Survival](/compendium/rules/skills.md#Survival) checks to navigate the ranger's demesne. On a failure, they are attacked by `dice:2d6|noform|noparens|avg` (`2d6`) winter wolves.  
...
- You are wreathed by a [fire shield](/compendium/spells/fire-shield.md). Enemies who strike the shield take `dice:4d8|noform|noparens|avg` (`4d8`) damage instead of `dice:2d8|noform|noparens|avg` (`2d8`).  
