...
Each lock has a creature molded in iron above it: a bat, snake, spider, or wolf, respectively. These locks can't be picked. If anything other than the correct key is placed inside a lock, creatures corresponding to the image above the lock (`dice:1d4|noform|avg` (`1d4`) [giant bats](compendium/bestiary/beast/giant-bat.md), `dice:1d4|noform|avg` (`1d4`) [giant poisonous snakes](compendium/bestiary/beast/giant-poisonous-snake.md), `dice:1d4|noform|avg` (`1d4`) [giant wolf spiders](compendium/bestiary/beast/giant-wolf-spider.md), or `dice:1d4|noform|avg` (`1d4`) [wolves](compendium/bestiary/beast/wolf.md)) are summoned into the room. Each summoned creature is hostile and disappears after 10 minutes or when reduced to 0 hit points. These beasts can't be [charmed](/compendium/rules/conditions.md#Charmed) or [frightened](/compendium/rules/conditions.md#Frightened).
...
Drawing a line from a spell to its material component crosses out letters that fall between the columns, as shown in diagram 4.4. Once all spells are connected to components, the untouched letters spell out "presto," which, when said aloud with the paper in hand, causes one or more [spell scrolls](compendium/items/spell-scroll-dmg.md) (or some other treasure of your choice) to magically appear.
