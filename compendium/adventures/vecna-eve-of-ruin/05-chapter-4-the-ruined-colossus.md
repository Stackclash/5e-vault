...
From this base, pilgrims venture out to find relics or lost warforged. At such times, `dice:2d4|noform|avg` (`2d4`) pilgrims (use the [warforged warrior](compendium/bestiary/construct/warforged-warrior-veor.md) stat block in appendix A) remain in Ialos to guard the mill or patrol its surroundings. When no expedition is underway, thirteen pilgrims (including their leader, Mercy, who also uses the [warforged warrior](compendium/bestiary/construct/warforged-warrior-veor.md) stat block) occupy Ialos. The pilgrims defend their home against attackers, but they flee when they're outmatched.
...
- **Skills.** The Docent has a +7 bonus to ability checks using one of the following skills (roll a `dice:d4|noform|avg` (`d4`)): (1) [Arcana](/compendium/rules/skills.md#Arcana), (2) [History](/compendium/rules/skills.md#History), (3) [Investigation](/compendium/rules/skills.md#Investigation), or (4) [Nature](/compendium/rules/skills.md#Nature).  
- **Spells.** The Docent knows one of the following spells and can cast it at will, requiring no spell components (roll a `dice:d6|noform|avg` (`d6`)): (1–2) [Detect Evil and Good](compendium/spells/detect-evil-and-good.md) or (3–6) [Detect Magic](compendium/spells/detect-magic.md). The Docent decides when to cast the spell.  
...
The easiest way to enter the colossus is through the mountain caves that lead to area L5. The characters can also search higher up the mountain along outdoor paths for the entrances to caverns that lead to areas L11 and L16. Searching for area L11 takes `dice:2d4|noform|avg` (`2d4`) hours, and during the characters' search, a hungry [roc](compendium/bestiary/monstrosity/roc.md) ambushes them. Searching for area L16 takes `dice:3d4|noform|avg` (`3d4`) hours, and during the characters' search, they stumble on a nest with two [rocs](compendium/bestiary/monstrosity/roc.md) and one roc egg the height of a human. The angry roc parents attack the characters, though the rocs don't pursue if the characters flee.
...
- **Psychic Damage.** For each ounce of graymatter fluid consumed, the creature takes `dice:2d10|noform|avg|text(11)` (`2d10`) psychic damage. A creature killed by this damage rises as a [zombie](compendium/bestiary/undead/zombie.md) `dice:1d4|noform|avg` (`1d4`) hours after dying.  
...
The chest is a [mimic](compendium/bestiary/monstrosity/mimic.md) in disguise. As soon as a creature touches it, the mimic attacks. Once it has grappled a creature, the mimic attempts to drag its victim outside so it can drop the victim off the edge of the walkway (area L22). Creatures that fall take `dice:13d6|noform|avg|text(45)` (`13d6`) bludgeoning damage.
